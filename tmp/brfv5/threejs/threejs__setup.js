...
export const create3DScene = (t3d, canvas) => {
  ...
  if (!SystemUtils.isWebGLSupported || !t3d) {
    return false
  }

  if (!t3d.hasOwnProperty('sceneScale')) {
    t3d.sceneScale = 2.0
  }
  ...
  const ambient = new AmbientLight(0xFFFFFF, 5.0)
...
export const setNumFaces = (t3d, numFaces) => {
  ...
  for (let i = baseNodes.length; i < numFaces; i++) {
    const baseNode = new Group()
    baseNode.name = 'baseNode_' + i
    baseNodes.push(baseNode)
    const modelNode = new Group()
...
export const set3DModelByName = (t3d, pathModel, modelName, onError) => {
  ...
  for (let i = 0; i < modelNodes.length; i++) {
    ...
    if (pathModel && modelName) {
      ...
      modelNode.traverse((child) => {
        if (foundModel && child === foundModel) {
          for (const subChild of foundModel.parent.children) {
    ...
    if (foundModel !== null) {
      foundModel.traverse((object) => {
        object.visible = true
      })
      foundModel.traverseAncestors((object) => {
        object.visible = true
      })
    }
  }
...
const adjustPlacement = (obj, data) => {
  ...
  if (rx !== 0 || ry !== 0 || rz !== 0)
    obj.rotation.set(toRadian(rx), toRadian(ry), toRadian(rz))
