...
export const updateByFace = (t3d, face, index, show) => {
  ...
  if (index >= baseNodes.length) {
    error(
      'threejs_brfv5_mapping.updateByFace: Call setNumFaces before loading the models and starting the tracking.'
    )
  ...
  if (show) {
    ...
    const scale = face.scale * si * 0.0133
    const x = -(face.translationX - cw * 0.5) * si
    let y = -(face.translationY - ch * 0.5) * si
    let rx = -face.rotationX * 1.3
    let ry = -face.rotationY * 1.3
    let rz = face.rotationZ

    const ryp = Math.abs(ry) / 30.0
    const rxp = Math.abs(rx) / 30.0
    ...
    if (!baseNode.visible || diffXAbs > 3 || diffYAbs > 3) {
      ...
      transform.x =
        transform.x +
        diffX * (diffXAbs < 1.0 ? 0.25 : diffXAbs < 2.0 ? 0.5 : 0.75)
      transform.y =
        transform.y +
        diffY * (diffYAbs < 1.0 ? 0.25 : diffYAbs < 2.0 ? 0.5 : 0.75)
    }
    ...
    transform.rx =
      transform.rx +
      diffRx * (diffRxAbs < 1.0 ? 0.25 : diffRxAbs < 2.0 ? 0.5 : 0.75)
    transform.ry =
      transform.ry +
      diffRy * (diffRyAbs < 1.0 ? 0.25 : diffRyAbs < 2.0 ? 0.5 : 0.75)
    transform.rz =
      transform.rz +
      diffRz * (diffRzAbs < 1.0 ? 0.25 : diffRzAbs < 2.0 ? 0.5 : 0.75)

    baseNode.position.set(transform.x, transform.y, transform.z)
    baseNode.rotation.set(
      toRadian(transform.rx),
      toRadian(transform.ry),
      toRadian(transform.rz)
    )
