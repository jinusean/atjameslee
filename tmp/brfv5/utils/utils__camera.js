...
const getUserMedia = (video) => {
  return new Promise((resolve, reject) => {
    window.navigator.mediaDevices
      .getUserMedia({ video: _videoConstraints })
      .then((mediaStream) => {
...
export const startCamera = (video, videoConstraints) => {
  if (
    !isVideo(
      video,
      logName +
        'startCamera: Provide a HTML video element to start the camera stream.'
    )
  ) {
    return
  }
  ...
  if (!videoConstraints) {
    videoConstraints = _default480p
  }

  if (videoConstraints.width) {
    _videoConstraints.width.ideal = videoConstraints.width
  }
  if (videoConstraints.height) {
    _videoConstraints.height.ideal = videoConstraints.height
  }
  if (videoConstraints.frameRate) {
    _videoConstraints.frameRate.ideal = videoConstraints.frameRate
  }
  if (videoConstraints.facingMode) {
    _videoConstraints.facingMode.ideal = videoConstraints.facingMode
  }
...
export const stopCamera = (video, pause = true, silent = false) => {
  ...
  if (
    !isVideo(
      video,
      logName +
        'stopCamera: Provide a HTML video element to stop the camera stream.'
    )
  ) {
    return
  }
...
export const toggleCameraFacingMode = (video) => {
  ...
  if (
    !isVideo(
      video,
      logName +
        'toggleCameraFacingMode: Provide a HTML video element to toggle the facing mode.'
    )
  ) {
    return
  }
  ...
  const newMode =
    _videoConstraints.facingMode.ideal === CameraFacingMode.USER
