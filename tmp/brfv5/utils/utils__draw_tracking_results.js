import { brfv5 } from '../brfv5__init.js'
import {
  drawCircles,
  drawRect,
  drawRects,
  drawTriangles,
} from './utils__canvas.js'
...
export const drawFaceDetectionResults = (brfv5Manager, brfv5Config, canvas) => {
  if (!brfv5Manager || !brfv5Config || !canvas) {
    return
  }
  ...
  drawRect(
    ctx,
    brfv5Config.faceDetectionConfig.regionOfInterest,
    colorSecondary,
    2.0
  )
...
export const drawFaceTrackingResultsDefault = (
  brfv5Manager,
  brfv5Config,
  canvas
) => {
  ...
  if (brfv5Config.enableFaceTracking) {
    ...
    for (let i = 0; i < faces.length; i++) {
      ...
      if (face.state === brfv5.BRFv5State.FACE_TRACKING) {
        drawRect(
          ctx,
          brfv5Config.faceTrackingConfig.regionOfInterest,
          colorPrimary,
          2.0
        )
