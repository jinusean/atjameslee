...
import { loadBRFv5Model } from 'brfv5-browser/js/brfv5/brfv5__init.js'
...
import {
  ...
  onEnterFrame,
  onExitFrame,
} from 'brfv5-browser/js/brfv5/brfv5__dynamic_performance.js'
...
import {
  ...
  getDataFromImage,
  closeImage,
} from 'brfv5-browser/js/ui/ui__input__image.js'
...
import {
  ...
  closeCamera,
  isVideoPaused,
} from 'brfv5-browser/js/ui/ui__input__camera.js'

import {
  startTracking,
  stopTracking,
} from 'brfv5-browser/js/ui/ui__input__data.js'

import {
  mountPNGOverlay,
  setSizePNGOverlay,
  hidePNGOverlay,
} from 'brfv5-browser/js/ui/ui__overlay__png.js'
import {
  mountTextureOverlay,
  setSizeTextureOverlay,
  hideTextureOverlay,
} from 'brfv5-browser/js/ui/ui__overlay__texture.js'
import {
  mountThreejsOverlay,
  setSizeThreejsOverlay,
  hideThreejsOverlay,
} from 'brfv5-browser/js/ui/ui__overlay__threejs.js'
import {
  mountTextureExporter,
  setSizeTextureExporter,
  hideTextureExporter,
} from 'brfv5-browser/js/ui/ui__exporter__texture.js'

import {
  mountPreloader,
  onProgress,
} from 'brfv5-browser/js/ui/ui__optional__preloader.js'
import {
  mountStats,
  updateStats,
} from 'brfv5-browser/js/ui/ui__optional__stats.js'
import {
  mountFullscreen,
  setFullscreenState,
} from 'brfv5-browser/js/ui/ui__optional__fullscreen.js'
import { mountStage } from 'brfv5-browser/js/ui/ui__stage.js'
import {
  ...
  setROIsWholeImage,
} from 'brfv5-browser/js/brfv5/brfv5__configure.js'
import {
  drawInput,
  drawInputMirrored,
} from 'brfv5-browser/js/utils/utils__canvas.js'
import { drawFaceTrackingResultsDefault } from 'brfv5-browser/js/utils/utils__draw_tracking_results.js'
import { getURLParameter } from 'brfv5-browser/js/utils/utils__get_params.js'
import { ScaleMode } from 'brfv5-browser/js/utils/utils__resize.js'
import { SystemUtils } from 'brfv5-browser/js/utils/utils__system.js'
import { log, error } from 'brfv5-browser/js/utils/utils__logging.js'
...
const _modelName =
  getURLParameter(window.location.search, 'type') === '42l' ? '42l' : '68l'
...
const _scaleMode = ScaleMode.PROPORTIONAL_INSIDE
const _t3d = {} // a ThreeJS namespace per view.
...
export const setupExample = (config = null) => {
  ...
  const container = document.getElementById('__brfv52')
  if (!container) {
    error('DIV with id __brfv52 is missing.')
    return
  }
  ...
  loadBRFv5Model(_modelName, _numChunksToLoad, 'models/', null, onProgress)
    ...
    .catch((e) => {
      error(
        'BRFV5_FAILED: WebAssembly supported: ',
        SystemUtils.isWebAssemblySupported,
        e
      )
    })
}
...
export const setExampleConfigValues = (config) => {
  ...
  if (config.numChunksToLoad >= 4) {
    _numChunksToLoad = config.numChunksToLoad
  }
  if (config.numFacesToTrack >= 0) {
    _numFacesToTrack = config.numFacesToTrack
  }
  if (config.enableDynamicPerformance) {
    _enableDynamicPerformance = config.enableDynamicPerformance
  }
  if (config.numTrackingPasses) {
    _numTrackingPasses = config.numTrackingPasses
  }
}
...
const initTracking = () => {
  log(
    _name + ': initTracking: ' + (_brfv5Config !== null && _width > 0),
    'config:',
    _brfv5Config,
    'width:',
    _width,
  ...
  if (_brfv5Config !== null && _width > 0) {
    ...
    if (_onConfigure) {
      _onConfigure(_brfv5Config, _t3d)
    }
...
const onImageDataUpdate = (
  imageData,
  activeCanvas,
  inactiveCanvas,
  trackOnlyOnce
) => {
  if (!_brfv5Manager || !imageData || !activeCanvas) {
    return
  }
  ...
  const numUpdates = !trackOnlyOnce
    ? 1
    : _brfv5Config.faceTrackingConfig.numFacesToTrack * 2
  ...
  if (_onTracking) {
    _onTracking(_brfv5Manager, _brfv5Config, activeCanvas, _t3d)
  } else {
    drawFaceTrackingResultsDefault(
      _brfv5Manager,
      _brfv5Config,
      activeCanvas,
      _t3d
  ...
  if (_brfv5Config.enableFaceTracking) {
    updateStats(
      averageTime.time,
      'numTrackingPasses: ' +
        _brfv5Config.faceTrackingConfig.numTrackingPasses +
        '<br />enableFreeRotation: ' +
        (_brfv5Config.faceTrackingConfig.enableFreeRotation ? 1 : 0)
    )
  } else {
